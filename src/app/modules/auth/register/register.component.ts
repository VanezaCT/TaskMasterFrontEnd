...
export class RegisterComponent {
  ...
  formRegister = new FormGroup({
    ...
    c_password: new FormControl('', [Validators.required, Validators.minLength(6), Validators.maxLength(20)]),
  });
  ...
  register() {
    ...
    if (this.formRegister.value.c_password != '' && this.formRegister.value.password != this.formRegister.value.c_password) {
    ...
    if (this.formRegister.valid) {
      ...
      this.userModel.c_password = this.formRegister.value.password;
  ...
  getErrorMessage(field: string) {
    ...
    if ((field == 'password') && (this.formRegister.get('password')?.hasError('required') ||
      ...
      return '*Contraseña entre 8 y 20 caracteres';

    if (field == 'c_password' && (this.formRegister.get('c_password')?.hasError('required')))
